...
def inpaint(img_ori, mask_ori):
    ...
    img_gray = torch.unsqueeze(torch.unsqueeze(torch.from_numpy(img_gray).float(), 0), 0).to(configs['DEVICE'])
    mask = torch.unsqueeze(torch.unsqueeze(torch.from_numpy(mask).float(), 0), 0).to(configs['DEVICE'])
...
if __name__ == "__main__":
    ...
    inpaint(IMG, MASK)
    # prepocess('examples/places2/images/places2_01.png', 'examples/places2/masks/places2_01.png')
