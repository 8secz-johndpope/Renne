...
def detect_pose(oriImg):
    ...
    for m in range(len(multiplier)):
        ...
        with torch.no_grad():
            Mconv7_stage6_L1, Mconv7_stage6_L2 = openpose(data)
    ...
    return generate_output(candidate, subset)

# Output
def generate_output(key_points, person_points):
    pose = np.zeros((person_points.shape[0], 18, 3))
    for ind, person in enumerate(person_points):
        for part in range(18):
            loc = int(person[part])
            if loc > 0:
                pose[ind][part] = np.append(key_points[loc][:2], 2.)
